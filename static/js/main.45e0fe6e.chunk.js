...
const DefaultProps = {
  ...
  url: "http://inu.ro"
}; // Queries the database for real data

const query = graphql_tag__WEBPACK_IMPORTED_MODULE_3___default()(_templateObject()); // Displays site info in the `<head>` with Helmet
...
const Home = props => {
  ...
  //
  // This is also a good example for typechecking / props
  // - Throughout the entire code we pass the same structure:
  // `useState(props)`, `setSiteInfo(data.generalSettings)`, `<SiteInfo {...siteInfo} />`
  // - And we decompose only when it is displayed in the `SiteInfo` component:
...
//# sourceMappingURL=main.45e0fe6e.chunk.js.map